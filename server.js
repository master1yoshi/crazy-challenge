...
// 3. LOGIQUE ET STOCKAGE DES SALONS
let rooms = {}; 
...
async function generateQuestionsWithAI(theme) {
    ...
    try {
        ...
        const prompt = `Generate a JSON array of 10 quiz questions in French about "${theme}". Structure: {"type": "${theme}", "text": "...", "options": ["A", "B", "C", "D"], "answer": 0}`;
...
function getFallbackQuestions(theme) {
    return Array.from({length: 10}, (_, i) => ({
        type: theme,
        text: `Question de secours ${i+1} sur ${theme}.`,
        options: ["A", "B", "C", "D"],
        answer: 0
    }));
}
...
io.on('connection', (socket) => {
    ...
    socket.on('reset_teams', () => { 
        if (!currentRoomId) return;
        getOrCreateRoom(currentRoomId).teams = {}; 
