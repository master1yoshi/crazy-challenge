...
// Connexion MySQL utilisant strictement tes variables d'environnement
...
db.connect((err) => {
    if (err) {
        console.error('Aiven Connection Error:', err);
    } else {
...
async function generateQuestionsWithAI(theme) {
    ...
    try {
        ...
        const questions = JSON.parse(result.response.text().trim());
...
function getFallbackQuestions(theme) {
    let list = [];
    for (let i = 1; i <= 10; i++) list.push({ type: theme, text: `Question de secours ${i}`, options: ["A", "B", "C", "D"], answer: 0 });
    return list;
}

app.get('/', (req, res) => res.send("<h1>Crazy Challenge en ligne</h1>"));
...
io.on('connection', (socket) => {
    ...
    socket.on('start_game_ai', async (theme) => {
        ...
        if (room.gameQuestions && room.gameQuestions.length > 0) { room.currentQuestionIndex = 0; sendQuestion(currentRoomId); }
    });
    socket.on('buzz', () => {
        if (!currentRoomId) return;
        const room = getOrCreateRoom(currentRoomId);
        if (room.canBuzz && !room.currentBuzzer && !room.buzzedThisRound.includes(socket.id)) {
            room.currentBuzzer = socket.id; room.canBuzz = false;
    ...
    socket.on('submit_answer', (choiceIndex) => {
        ...
        if (choiceIndex === q.answer) {
            ...
            if (room.buzzedThisRound.length >= Object.keys(room.teams).length) {
                setTimeout(() => nextQuestion(currentRoomId), 2000);
            } else { room.canBuzz = true; io.to(currentRoomId).emit('release_buzz', { buzzedThisRound: room.buzzedThisRound }); }
        }
        io.to(currentRoomId).emit('update_teams', room.teams);
    });
    socket.on('disconnect', () => {
        ...
        if (room && room.teams[socket.id]) { delete room.teams[socket.id]; io.to(currentRoomId).emit('update_teams', room.teams); }
    });
});

function sendQuestion(roomId) {
    const room = rooms[roomId]; room.canBuzz = false;
    const q = room.gameQuestions[room.currentQuestionIndex];
    io.to(roomId).emit('new_question_intro', { type: q.type, text: q.text });
    setTimeout(() => { room.canBuzz = true; io.to(roomId).emit('show_options', { options: q.options }); }, 3000);
}
