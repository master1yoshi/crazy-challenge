...
socket.on('change_theme', (newTheme) => {
    if (questionsData[newTheme]) {
        const room = getOrCreateRoom(currentRoomId);
        // Mélanger et prendre 20 questions
        const all = [...questionsData[newTheme]];
        room.gameQuestions = all.sort(() => 0.5 - Math.random()).slice(0, 20);
        room.currentQuestionIndex = 0;
        
        io.to(currentRoomId).emit('status_message', `Le Host a changé le thème. Nouveau thème : ${newTheme.toUpperCase()}`);
...
// Dans la partie io.on('connection', ...)
socket.on('change_theme', (theme) => {
    if (questionsData[theme]) {
        const room = getOrCreateRoom(currentRoomId);
        
        // Mélange les questions du thème et en prend 20
        const allQuestions = [...questionsData[theme]];
        room.gameQuestions = allQuestions.sort(() => 0.5 - Math.random()).slice(0, 20);
        room.currentQuestionIndex = 0;
